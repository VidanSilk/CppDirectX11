...
{	//사용할 레이어 등록
	...
	CreateRenderLayer("Floor", 10);
	//CreateRenderLayer("EffectBack", -50);
	CreateRenderLayer("EffectFront", 50);
...
{
	const float sz = Src->GetWorldPosition().z;
	const float dz = Dest->GetWorldPosition().z;

	//  z 우선 정렬 (작은 z 먼저 그리고, 큰 z를 나중에 그려서 위로)
	if (fabs(sz - dz) > 0.0001f) return sz < dz;
