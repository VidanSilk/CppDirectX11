...
//스페이스바 추가  1208
void CInput::ChangeKeySpaceBar(const std::string& Name, bool Space)
{
	FBindKey* Key = FindBindKey(Name);
...
void CInput::UpdateInput(float DeltaTime)
{
	...
	case EInputSystem_Type::DInput:
		...
		//1209 Space
		if (mKeyState[DIK_SPACE] & 0x80) {
			
			//스페이스바 
			if (!mSpace[EInputType::Down] && !mSpace[EInputType::Hold])
			{
				mSpace[EInputType::Down] = true;
				mSpace[EInputType::Hold] = true;
			}
			else
			{
				mSpace[EInputType::Down] = false; //수정 
				//mSpace[EInputType::Down] = true;
			}
		}
		else if (mSpace[EInputType::Hold])
		{
			mSpace[EInputType::Hold] = false;
			mSpace[EInputType::Down] = false;
			mSpace[EInputType::Up] = true;
		}
		else if (mSpace[EInputType::Up])
		{
			mSpace[EInputType::Up] = false;
		}
		else {//바인드 동작이 없을대?
			mSpace[EInputType::Down] = false;
			mSpace[EInputType::Hold] = false;
...
void CInput::UpdateBind(float DeltaTime)
{
	...
	for (; iter != iterEnd; ++iter)
	{
		...
		if (iter->second->Key->Down &&
			...
			iter->second->Space == mSpace[EInputType::Hold])
		{
		...
		//스페이스추가 1209
		if (iter->second->Space) {
		
			if (mSpace[EInputType::Up]) {
				Verification = true;
			}
